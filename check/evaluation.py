...
for idx, outline in enumerate(outlines):
    ...
    if stats:
        if outline.startswith('SoPlex status') and not instances[instancename]['status'] == 'fail':
            ...
            elif outline.find('error') >= 0:
                instances[instancename]['status'] = 'error'
            else:
            ...
            instances[instancename]['sparsityobj'] = float(outlines[idx+15].split()[3])
            instances[instancename]['sparsitycomb'] = float(outlines[idx+16].split()[3])
            instances[instancename]['minabsval'] = float(outlines[idx+17].split()[4])
            instances[instancename]['maxabsval'] = float(outlines[idx+18].split()[4])
            ...
            instances[instancename]['qpbacksteps'] = int(outlines[idx+7].split()[-1])
            instances[instancename]['qpresolves'] = int(outlines[idx+8].split()[-1])
...
if check_solu:
    ...
    for soluline in solufile:
        ...
        if name in instances:
            ...
            if not instances[name]['status'] in ['timeout', 'fail', 'abort', 'error']:
...
errors = sum(1 for name in instances if instances[name]['status'] == 'error')
...
print('{} total: {} optimal, {} infeasible, {} unbounded, {} timeouts, {} inconsistents, {} fails, {} errors, {} aborts'.format(len(instances),optimal,infeasible,unbounded,timeouts,inconsistents,fails,errors,aborts))
