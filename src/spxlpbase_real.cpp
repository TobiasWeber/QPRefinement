   ...
   DIdxSet*      p_intvars)              ///< integer variables.
{
   ...
   // set offset to zero (in case it is not set while reading)
   changeObjOffset(0.0);
...
/// Process RHS section.
static void MPSreadRhs(MPSInput& mps, LPRowSetBase<Real>& rset, const NameSet& rnames, SPxOut* spxout, Real *offset)
{
   ...
   while( mps.readLine() )
   {
      if( mps.field0() != 0 )
      {
         ...
         else if( !strcmp(mps.field0(), "QUADOBJ") )
            mps.setSection(MPSInput::QUADOBJ);
      ...
      else
      {
         if( (idx = rnames.number(mps.field2())) < 0 )
         {
            // Constant offset of objective saved in QPS as RHS value for the "row" objective (negative for positve offset!)
            if( !strcmp(mps.field2(), mps.objName()) )
               *offset = -1*atof(mps.field3());
            else
...
/// Process QUADOBJ section of the qps file
/**
 * The specification is taken from the paper "A Repository of Convex Quadratic Programming Problems"
 * by Istv ́an Maros and Csaba M ́esz ́aros (1997). It is available at
 * https://www.doc.ic.ac.uk/research/technicalreports/1997/DTR97-6.pdf .
 * QPS files are a simple extension of MPS files that store the quadratic objective matrix in an extra section
 * at the end of the file.
*/
static void MPSreadQuadObj(MPSInput& mps, QPMatrixBase<Real>& qpset, NameSet& cnames, SPxOut* spxout)
{
   Real val;
   int idx;
   int col_idx = 0;
   int var_num = cnames.num();
   char colname[MPSInput::MAX_LINE_LEN] = { '\0' };
   DSVectorBase<Real> vec;
   ...
   while( mps.readLine() )
   {
      if( mps.field0() != 0 )
      {
	    ...
	    vec.clear();
         ...
         // last column was final one?
         for( int j=col_idx; j < var_num; ++j )
         {
            qpset.add(vec);
            col_idx++;
         }
      ...
      if( strcmp(colname, mps.field1()) )
      {
         ...
         int new_col_idx = cnames.number(mps.field1());

         // new column not next column (variable)?
         for( int j=col_idx; j < new_col_idx; ++j )
         {
            qpset.add(vec);
            col_idx++;
         }
      ...
      if( (idx = cnames.number(mps.field2())) < 0 )
         mps.entryIgnored("Column", mps.field1(), "column", mps.field2());
      else if( val != 0.0 )
         vec.add(idx, val);
      ...
      if( mps.field5() != 0 )
      {
         ...
         if( (idx = cnames.number(mps.field4())) < 0 )
            mps.entryIgnored("Column", mps.field1(), "column", mps.field4());
         else if( val != 0.0 )
            vec.add(idx, val);
   ...
   DIdxSet*      p_intvars)         ///< integer variables.
{
   ...
   if( mps.section() == MPSInput::RHS )
      MPSreadRhs(mps, rset, *rnames, spxout, &offset);
   ...
   if( mps.section() == MPSInput::QUADOBJ )
      MPSreadQuadObj(mps, qpset, *cnames, spxout);
   ...
   ) const
{
   ...
   if( QPMatrixBase::num() > 0 )
   {
      MPSwriteRecord(p_output, 0, "RHS", "MINIMIZE", offset);
   }
   ...
   if( QPMatrixBase::num() > 0 )
   {
      p_output << "QAUDOBJ" << std::endl;

      for( i = 0; i < nCols(); i++ )
      {

         const SVectorBase<Real>& col = rowcolVector(i);
         int colsize2 = (col.size() / 2) * 2;
         ...
         for( k = 0; k < colsize2; k += 2 )
            MPSwriteRecord(p_output, 0, getColName(*this, i, p_cnames, name),
               getColName(*this, col.index(k), p_cnames, name1), col.value(k),
               getColName(*this, col.index(k + 1), p_cnames, name2), col.value(k + 1));
