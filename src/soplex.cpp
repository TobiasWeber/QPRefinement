...
namespace soplex
{
   ...
   public:
      ...
      Settings()
      {
         if( !_defaultsAndBoundsInitialized )
         {
            ...
            _boolParamName[SoPlex::RELIABLERESOLVE] = "reliableresolve";
            _boolParamDescription[SoPlex::RELIABLERESOLVE] = "solve with reliable settings if fast settings for qpOASES fail?";
            _boolParamDefault[SoPlex::RELIABLERESOLVE] = true;

            // use full linear independence test in qpOASES?
            _boolParamName[SoPlex::LITESTFULL] = "litestfull";
            _boolParamDescription[SoPlex::LITESTFULL] = "enable fulll linear independence test in qpOASES?";
            _boolParamDefault[SoPlex::LITESTFULL] = false;
            ...
            _intParamName[SoPlex::QP_REF_MAX_NUM_BACKSTEP] = "qp_ref_max_num_backstep";
            _intParamDescription[SoPlex::QP_REF_MAX_NUM_BACKSTEP] = "maximal number of steps back in qp refinement (min 0, max 20, default 5)";
            _intParamLower[SoPlex::QP_REF_MAX_NUM_BACKSTEP] = 0;
            _intParamUpper[SoPlex::QP_REF_MAX_NUM_BACKSTEP] = 20;
            _intParamDefault[SoPlex::QP_REF_MAX_NUM_BACKSTEP] = 5;

            // settings to hand to qpOASES
            _intParamName[SoPlex::QPOASES_SETTINGS] = "qpoases_settings";
            _intParamDescription[SoPlex::QPOASES_SETTINGS] = "qpOASES settings (default 0* (=QPOASES_SETTINGS_DEFAULT), fast (MPC) 1, qpOASES default 2, reliable 3)";
            _intParamLower[SoPlex::QPOASES_SETTINGS] = 0;
            _intParamUpper[SoPlex::QPOASES_SETTINGS] = 3;
            _intParamDefault[SoPlex::QPOASES_SETTINGS] = SoPlex::QPOASES_SETTINGS_DEFAULT;

            // QP solver
            _intParamName[SoPlex::QP_SOLVER] = "qp_solver";
            _intParamDescription[SoPlex::QP_SOLVER] = "qp solver to pick (0 - qpOASES, 1 - QORE)";
            _intParamLower[SoPlex::QP_SOLVER] = 0;
            _intParamUpper[SoPlex::QP_SOLVER] = 1;
            _intParamDefault[SoPlex::QP_SOLVER] = 0;
            ...
            _realParamName[SoPlex::QP_SPARSITY_THRESHOLD] = "qp_sparsity_threshold";
            _realParamDescription[SoPlex::QP_SPARSITY_THRESHOLD] = "threshold that triggers sparse qp solving (lower 0.0, upper 1.0, default 0.0)";
            _realParamLower[SoPlex::QP_SPARSITY_THRESHOLD] = 0.0;
            _realParamUpper[SoPlex::QP_SPARSITY_THRESHOLD] = 1.0;
            _realParamDefault[SoPlex::QP_SPARSITY_THRESHOLD] = 0.0;
   ...
   const SVectorRational& SoPlex::rowcolVectorRational(int i) const
   {
      assert(_rationalLP != 0);
      return _rationalLP->rowcolVector(i);
   }
#endif
      ...
      else
      {
         ...
         for( int col = 0; col < numCols; col++ )
         {
            ...
            if( _basisStatusCols[col] == SPxSolver::BASIC )
            {
               ...
               if( row >= numRows )
                  file << " BS ";
               else if( _basisStatusRows[row] == SPxSolver::ON_UPPER && (!cpxFormat || _rowTypes[row] == SoPlex::RANGETYPE_BOXED) )
                  file << " XU ";
               else
                  file << " XL ";
   ...
   bool SoPlex::setIntParam(const IntParam param, const int value, const bool quiet, const bool init)
   {
      ...
      // maximal number of steps back in qp refinement
      case SoPlex::QP_REF_MAX_NUM_BACKSTEP:
         break;

      // settings to hand to qpOASES
      case SoPlex::QPOASES_SETTINGS:
         break;

      // choose between QP solvers
      case SoPlex::QP_SOLVER:
         break;
   ...
   bool SoPlex::setRealParam(const RealParam param, const Real value, const bool quiet, const bool init)
   {
      ...
      // threshold that triggers sparse qp solving, if sparcity of qp matrices is smaller
      case SoPlex::QP_SPARSITY_THRESHOLD:
         break;
