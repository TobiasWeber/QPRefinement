...
public:
   ...
   /// returns vector of qp matrix \p i
   const SVectorRational& rowcolVectorRational(int i) const;
#endif
   ...
   /// solves rational QP with iterative refinement
   SPxSolver::Status solveQP();
   ...
   typedef enum
   {
      ...
      /// use reliable resolve if fast solve fails?
      RELIABLERESOLVE = 10,

      /// use full linear independence test in qpOASES?
      LITESTFULL = 11,
   ...
   typedef enum
   {
      ...
      /// maximal number of steps back in qp refinement
      QP_REF_MAX_NUM_BACKSTEP = 24,

      /// settings handed to qpOASES
      QPOASES_SETTINGS = 25,
...
#if defined(SOPLEX_WITH_QPOASES) || defined(SOPLEX_WITH_QORE)
   /// values for parameter QPOASES_SETTINGS
   enum
   {
      /// defaul settings for qpOASES in refinment
      QPOASES_SETTINGS_DEFAULT = 0,

      /// fast settings for qpOASES in refinment (qpOASES MPC)
      QPOASES_SETTINGS_FAST = 1,

      /// medium settings for qpOASES in refinment (qpOASES default)
      QPOASES_SETTINGS_MEDIUM = 2,

      /// reliable settings for qpOASES in refinment (qpOASES reliable)
      QPOASES_SETTINGS_RELIABLE = 3
   };

   /// values for parameters QP_SOLVER
   enum
   {
      /// qpOASES
      SOLVER_QPOASES = 0,
   ...
   typedef enum
   {
      ...
      /// threshold that triggers sparse qp solving, if sparcity of qp matrices is smaller
      QP_SPARSITY_THRESHOLD = 23,
...
private:
   ...
   //**@name Quadratic programm data */
   //@{
#ifdef SOPLEX_WITH_QPOASES

   /// qpOASES interface variables
#ifdef SOPLEX_WITH_QPOASES_SPARSE
   qpOASES::sparse_int_t* H_jc = 0;
   qpOASES::sparse_int_t* H_ir = 0;
   qpOASES::real_t* H_val = 0;
   qpOASES::SymSparseMat* H_spar = 0;
   qpOASES::sparse_int_t* A_jc = 0;
   qpOASES::sparse_int_t* A_ir = 0;
   qpOASES::real_t* A_val = 0;
   qpOASES::SparseMatrix* A_spar = 0;
#endif
   qpOASES::real_t* H = 0;
   qpOASES::real_t* A = 0;
   qpOASES::real_t* g = 0;
   qpOASES::real_t* lb = 0;
   qpOASES::real_t* ub = 0;
   qpOASES::real_t* lbA = 0;
   qpOASES::real_t* ubA = 0;
   ...
   //**@name Private solving methods implemented in solveqp.cpp */
   //@{
#if defined(SOPLEX_WITH_QPOASES) || defined(SOPLEX_WITH_QORE)
   /// computes and loads rational QP-basis into rationalLUSolver
   void _computeAndLoadRationalQPBasis(bool& error, DataArray< SPxSolver::VarStatus >& basisStatusRows, DataArray< SPxSolver::VarStatus >& basisStatusCols, DVectorRational& righthandside, int& matrixdim, int& numBasicCols, int& numBasicRows);
#ifdef SOPLEX_WITH_QPOASES
   /// Copies the QP from soplex into qpOASES
   void _copyQPintoqpOASES(const int var_num, const int constr_num);

   /// Copies the solution of the QP from qpOASES into soplex
   void _copySolintoSPX(const qpOASES::QProblem& qp, VectorReal& primal, VectorReal& dual, const int var_num, const int constr_num);

   /// call qpOASES to resolve real QP
   SPxSolver::Status _resolveRealQP( qpOASES::QProblem& soplex_ir_qp, const bool acceptUnbounded, const bool acceptInfeasible,
                                      VectorReal& primal,VectorReal& dual, DataArray< SPxSolver::VarStatus >& basisStatusRows,
                                      DataArray< SPxSolver::VarStatus >& basisStatusCols, bool& returnedBasis,
                                      const bool forceNoSimplifier = false );

   /// call qpOASES to solve real QP
   SPxSolver::Status _solveRealQP( qpOASES::QProblem& soplex_ir_qp, const bool acceptUnbounded, const bool acceptInfeasible,
                                      VectorReal& primal, VectorReal& dual, DataArray< SPxSolver::VarStatus >& basisStatusRows,
                                      DataArray< SPxSolver::VarStatus >& basisStatusCols, bool& returnedBasis,
                                      const bool forceNoSimplifier = false );

   /// set appropriate options for all qpOASES calls
   void _setQPoasesOptions(qpOASES::Options& options, const bool reliable);
#endif
#ifdef SOPLEX_WITH_QORE
   /// Copies the QP from soplex into QORE
   void _copyQPintoQORE(const int var_num, const int constr_num);

   /// Copies the solution of the QP from QORE into soplex
   void _copySolintoSPX( QoreProblem* pproblem, VectorReal& primal, VectorReal& dual, const int var_num, const int constr_num);

   /// call QORE to solve real QP
   SPxSolver::Status _solveRealQP( QoreProblem* pproblem, const bool acceptUnbounded, const bool acceptInfeasible,
                                      VectorReal& primal, VectorReal& dual, DataArray< SPxSolver::VarStatus >& basisStatusRows,
                                      DataArray< SPxSolver::VarStatus >& basisStatusCols, bool& returnedBasis,
                                      const bool forceNoSimplifier = false );

   /// call QORE to resolve real QP
   SPxSolver::Status _resolveRealQP( QoreProblem* pproblem, const bool acceptUnbounded, const bool acceptInfeasible,
                                      VectorReal& primal,VectorReal& dual, DataArray< SPxSolver::VarStatus >& basisStatusRows,
                                      DataArray< SPxSolver::VarStatus >& basisStatusCols, bool& returnedBasis,
                                      const bool forceNoSimplifier = false );
#endif
   /// solves current QP problem with iterative refinement
   void _performOptIRQP(SolRational& sol, bool acceptUnbounded, bool acceptInfeasible, int minRounds,
   ...
   void _realSolToRational( SolRational& sol, DVectorReal& primalReal, DVectorReal& dualReal);

   /// computes primal violation of current sulution to the refined QP
   void _primViolation( SolRational& sol, Rational& boundsViolation, Rational& sideViolation );

   /// computes dual violation of current solution to the refined QP
   void _dualViolation( SolRational& sol, Rational& redCostViolation, Rational& dualViolation, const bool maximizing );

   /// applies primal scaling factor to bounds and sides of QP
   void _appPrimScale( Rational& primalScale );

   /// applies dual scaling factor to objective function of QP
   void _appDualScale( SolRational& sol, Rational& dualScale );

   /// correct primal solution
   int _corrPrimSol(SolRational& sol, Rational& primalScale, const int maxDimRational, DVectorReal& primalReal);

   /// debug: calculate reduced cost and dual violation
   void _dbgRedAndDualViolation(SolRational& sol, DVectorReal& dualReal, DVectorReal& primalReal, const bool maximizing, Rational& dualScale);

   /// correct dual solution
   int _corrDualSol(SolRational& sol, Rational& dualScale, const int maxDimRational, DVectorReal& dualReal);

   /// factorizes rational basis matrix in column representation for qp
   void _factorizeColumnRowRational_qp(SolRational& sol, DataArray< SPxSolver::VarStatus >& basisStatusRows, DataArray< SPxSolver::VarStatus >& basisStatusCols, bool& stoppedTime, bool& stoppedIter, bool& error, bool& optimal);
