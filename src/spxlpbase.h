 ...
 */
template < class R >
class SPxLPBase : protected LPRowSetBase<R>, protected LPColSetBase<R>, protected QPMatrixBase<R>
{
...
public:
   ...
   /// Returns number of columns/rows in QP matrix.
   int nRowcols() const
   {
      return QPMatrixBase<R>::num();
   }
   ...
   /// Returns number of nonzeros in QP matrix.
   int nNzos_qp() const
   {
      return QPMatrixBase<R>::nNzos();
   }
   ...
   /// Returns row/column vector of row/column \p i of qp objective matrix.
   const SVectorBase<R>& rowcolVector(int i) const
   {
      return QPMatrixBase<R>::rowcolVector(i);
   }

   /// Returns row/column vector with identifier \p id of qp objective matrix.
   const SVectorBase<R>& rowcolVector(const SPxColId& id) const
   {
      return QPMatrixBase<R>::rowcolVector(id);
   }
   ...
   virtual void addRowcol(const SVectorBase<R>& rowcol)
   {
      doAddRowcol(rowcol);
   }
   ...
   ///
   template < class S >
   void addRowcol(const S* rowcolValues, const int* rowcolIndices, int rowcolSize)
   {
      QPMatrixBase<R>::add(rowcolValues, rowcolIndices, rowcolSize);
   }

   /// Adds \p rowcol to QPMatrixBase.
   virtual void addRowcol(SPxColId& id, const SVectorBase<R>& rowcol)
   {
      addRowcol(rowcol);
      id = cId(nCols() - 1);
   }
   ...
   ///
   virtual void addRowcols(const SVSetBase<R>& pset)
   {
      doAddRowcols(pset);
   }
   ...
   ///
   template < class S >
   void addRowcols(const S* rowcolValues, const int* rowcolIndices, const int* rowcolStarts, const int* rowcolLengths, const int numRowcols, const int numValues)
   {
      assert(numValues <= 0 || rowcolValues != 0);
      assert(numValues <= 0 || rowcolIndices != 0);
      assert(numValues <= 0 || rowcolStarts != 0);
      assert(numValues <= 0 || rowcolLengths != 0);

      int oldRowcolNumber = nRowcols();

      QPMatrixBase<R>::memRemax(oldRowcolNumber + numRowcols);
      for( int i = 0; i < numRowcols; i++ )
      {
         assert(numValues <= 0 || rowcolStarts[i] + rowcolLengths[i] <= numValues);
         if( numValues <= 0 )
            QPMatrixBase<R>::add( (S*)0, (int*)0, 0 );
         else
            QPMatrixBase<R>::add( &(rowcolValues[rowcolStarts[i]]), &(rowcolIndices[rowcolStarts[i]]), rowcolLengths[i] );
      }
   }

   /// Adds all SVectorBase%s of \p set to QPMatrixBase.
   virtual void addRowcols(SPxColId id[], const SVSetBase<R>& set)
   {

      int i = nRowcols();
      addRowcols(set);
      for( int j = 0; i < nRowcols(); ++i, ++j )
         id[j] = cId(i);
   }
   ...
   void printProblemStatistics(std::ostream& os)
   {
      ...
      os << "  Columns           : " << nCols() << "\n"
         ...
         << " sparsity objective : " << Real(nNzos_qp()) / Real(nCols()) / Real(nCols()) << "\n"
         << "  sparsity combined : " << (Real(nNzos()) / Real(nCols()) / Real(nRows()) + Real(nNzos_qp()) / Real(nCols()) / Real(nCols()))/2 << "\n"
#endif
...
#if defined(SOPLEX_WITH_QPOASES) || defined(SOPLEX_WITH_QORE)
   /// Updates linearization of quadratic term in objective for a given primal vector, i.e., Q x;
   /// @throw SPxInternalCodeException if dimension of primal vector does not match number of columns or if the dimension of
   ///        the linearization vector does not match the number of columns
   virtual void addPrimalQPlin(const VectorBase<R>& primal, VectorBase<R>& linearization) const
   {
      if( primal.dim() != nCols() )
      {
         throw SPxInternalCodeException("XSPXLP05 Primal vector for computing linearization has wrong dimension");
      }

      if( linearization.dim() != nCols() )
      {
         throw SPxInternalCodeException("XSPXLP06 Linearization vector computing linearization has wrong dimension");
      }

      QPMatrixBase<R>::addQPMatrixvectorproduct(primal, linearization);
   }

   /// Updates linearization of quadratic term in objective for a given primal vector, i.e., Q x;
   /// @throw SPxInternalCodeException if dimension of primal vector does not match number of columns
   R PrimalQPlinentry(const VectorBase<R>& primal, const int index )
   {
      assert( index >= 0);
      if( primal.dim() != nCols() )
      {
         throw SPxInternalCodeException("XSPXLP05 Primal vector for computing linearization entry has wrong dimension");
      }

      return QPMatrixBase<R>::QPMatrixvectorproductentry(primal, index);
   }
#endif
   ...
   bool isConsistent() const
   {
      ...
      return LPRowSetBase<R>::isConsistent() && LPColSetBase<R>::isConsistent() && QPMatrixBase<R>::isConsistent();
#else
      return true;
#endif
   }
...
private:
   ...
   {
      return QPMatrixBase<R>::rowcolVector_w(i);
   }
   ...
   ///
   void doAddRowcol(const SVectorBase<R>& rowcol)
   {
      QPMatrixBase<R>::add(rowcol);
   }

   ///
   void doAddRowcols(const SVSetBase<R>& rowcol)
   {
      QPMatrixBase<R>::add(rowcol);
   }
