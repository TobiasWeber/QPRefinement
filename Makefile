...
LINKSMARKERFILE	=	$(LIBDIR)/linkscreated.$(OSTYPE).$(ARCH).$(COMP)$(LINKLIBSUFFIX).$(EGLIB).$(QPOASES).$(QORE)
...
QPDEP	:=	$(SRCDIR)/depend.qp
QPSRC	:=	$(shell cat $(QPDEP))
ifeq ($(QPOASES),dense)
ifeq ($(LEGACY),false)
CPPFLAGS	+=	-DSOPLEX_WITH_QPOASES -I$(LIBDIR)/qpoases.$(OSTYPE).$(ARCH).$(COMP)/include
LDFLAGS		+=	$(LIBDIR)/qpoases.$(OSTYPE).$(ARCH).$(COMP)/bin/libqpOASES.a -llapack -lblas -lm
SOFTLINKS	+=	$(LIBDIR)/qpoases.$(OSTYPE).$(ARCH).$(COMP)
LINKSINFO	+=	"\n  -> \"qpoases.$(OSTYPE).$(ARCH).$(COMP)\" is a directory containing the qpOASES installation, i.e., \"qpoases.$(OSTYPE).$(ARCH).$(COMP)/include/qpOASES.hpp\" and \"qpoases.$(OSTYPE).$(ARCH).$(COMP)/bin/libqpOASES.so\" should exist.\n"
endif
endif
ifeq ($(QPOASES),sparse)
ifeq ($(LEGACY),false)
CPPFLAGS	+=	-DSOPLEX_WITH_QPOASES -DSOPLEX_WITH_QPOASES_SPARSE -I$(LIBDIR)/qpoases.$(OSTYPE).$(ARCH).$(COMP)/include
LDFLAGS		+=	$(LIBDIR)/qpoases.$(OSTYPE).$(ARCH).$(COMP)/bin/libqpOASES.a -llapack -lblas -lm
SOFTLINKS	+=	$(LIBDIR)/qpoases.$(OSTYPE).$(ARCH).$(COMP)
LINKSINFO	+=	"\n  -> \"qpoases.$(OSTYPE).$(ARCH).$(COMP)\" is a directory containing the qpOASES installation, i.e., \"qpoases.$(OSTYPE).$(ARCH).$(COMP)/include/qpOASES.hpp\" and \"qpoases.$(OSTYPE).$(ARCH).$(COMP)/bin/libqpOASES.so\" should exist.\n"
endif
...
ifeq ($(QORE),true)
ifeq ($(LEGACY),false)
CPPFLAGS	+=	-DSOPLEX_WITH_QORE -I$(LIBDIR)/qore.deploy.$(OSTYPE).$(ARCH).$(COMP)/include
LDFLAGS		+=	$(LIBDIR)/qore.deploy.$(OSTYPE).$(ARCH).$(COMP)/lib64/libqpsolver.a $(LIBDIR)/qore.deploy.$(OSTYPE).$(ARCH).$(COMP)/lib64/libkktpack.a $(LIBDIR)/qore.deploy.$(OSTYPE).$(ARCH).$(COMP)/lib64/libqpcore.a $(LIBDIR)/qore.deploy.$(OSTYPE).$(ARCH).$(COMP)/lib64/libspmatrix.a -llapack -lblas -lumfpack -lm
SOFTLINKS	+=	$(LIBDIR)/qore.deploy.$(OSTYPE).$(ARCH).$(COMP)
LINKSINFO	+=	"\n  -> \"qore.deploy.$(OSTYPE).$(ARCH).$(COMP)\" is the QORE/deploy directory, i.e., \"qore.deploy.$(OSTYPE).$(ARCH).$(COMP)/include/qpsolver.h\" and \"qore.deploy.$(OSTYPE).$(ARCH).$(COMP)/bin/libqpsolver.a\" should exist.\n"
endif
endif
...
qtest:
ifeq ($(LEGACY),false)
		cd check; ./qtest.sh $(TEST) ../$(BINFILE) $(SETTINGS) $(TIME) $(RESDIR)
endif
...
depend:
		...
		@echo `grep -l "SOPLEX_WITH_QPOASES" $(ALLSRC)` >$(QPDEP)
		@echo `grep -l "SOPLEX_WITH_QORE" $(ALLSRC)` >>$(QPDEP)
...
.PHONY: touchexternal
touchexternal:	$(GMPDEP) $(ZLIBDEP) $(EGLIBDEP) $(QPSDEP) $(PARASCIPDEP) $(LEGACYDEP) | $(OBJDIR)
...
ifneq ($(QPOASES),$(LAST_QPOASES))
		@-touch $(QPSRC)
endif
ifneq ($(QORE),$(LAST_QORE))
		@-touch $(QPSRC)
endif
...
ifneq ($(USRARFLAGS),$(LAST_USRARFLAGS))
		...
		@echo "LAST_QPOASES=$(QPOASES)" >> $(LASTSETTINGS)
		@echo "LAST_QORE=$(QORE)" >> $(LASTSETTINGS)
...
echosoftlinks:
		@echo
		@echo "- Current settings: OSTYPE=$(OSTYPE) ARCH=$(ARCH) COMP=$(COMP) SUFFIX=$(LINKLIBSUFFIX) EGLIB=$(EGLIB) QPOASES=$(QPOASES) QORE=$(QORE)"
		@echo
...
ifneq ($(QPOASES),off)
ifneq ($(QPOASES),dense)
ifneq ($(QPOASES),sparse)
		$(error invalid QPOASES flag selected: QPOASES=$(QPOASES). Possible options are: off dense sparse)
endif
endif
endif
ifneq ($(QORE),true)
ifneq ($(QORE),false)
		$(error invalid QORE flag selected: QORE=$(QORE). Possible options are: true false)
endif
endif
ifneq ($(PARASCIP),true)
